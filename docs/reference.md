...
- [SPEC.md](SPEC.md) — model metadata schema (capabilities, freethought, strengths/weaknesses, throughput), the llama-swap metadata channel, and the vLLM backend (`template: vllm` / `template: vllm-docker`)
